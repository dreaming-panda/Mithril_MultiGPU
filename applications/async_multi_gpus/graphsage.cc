...
class GraphSage: public AbstractApplication {
    ...
    public:
        GraphSage(int num_layers, int num_hidden_units, int num_classes, int num_features, double dropout_rate, bool use_residual, bool multi_label):
            AbstractApplication(num_features),
            num_layers_(num_layers), num_hidden_units_(num_hidden_units), num_classes_(num_classes), dropout_rate_(dropout_rate), use_residual_(use_residual), multi_label_(multi_label) {
                assert(num_layers >= 1);
...
int main(int argc, char ** argv) {
    ...
    GraphSage * sage = new GraphSage(num_layers, num_hidden_units, num_classes, num_features, dropout, use_residual, multi_label);
