...
class GIN: public AbstractApplication {
    private:
        int num_layers_;
    ...
    public:
        GIN(int num_layers, int num_hidden_units, int num_classes, int num_features, double dropout_rate, bool use_residual): 
        ...
        Tensor * forward(Tensor * input) {
            ...
            for (int i = 0; i < num_layers_; ++ i) {
                Tensor * nbr_sum = aggregation(t, SUM);
                t = add(t, nbr_sum, 1., 1., true);
                // MLP
...
int main(int argc, char ** argv) {
    ...
    if (! node_id) {
        ...
        printf("The number of GIN layers: %d\n", num_layers);
    ...
    GIN * gin = new GIN(num_layers, num_hidden_units, num_classes, num_features, dropout, use_residual);
    ...
    execution_engine->set_aggregation_type(SUM);
    ...
    if (partition_strategy == "hybrid") {
        ...
        CUDAModelPartitioning partition = ModelPartitioner::get_model_parallel_partition(
                gin, num_stages, num_layers, cost_each_layer, num_vertices
                );
    ...
    // model training
    execution_engine->execute_application(gin, num_epoch);
