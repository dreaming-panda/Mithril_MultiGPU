...
hostfile=./nsdi2023/weak_scaling/pipeline/hostfile
mpirun nvidia-smi
...
dataset_path=/shared_hdd_storage/jingjichen/gnn_datasets/pipeline_parallel_datasets
...
do
    ...
    do
        mkdir -p ./nsdi2023/weak_scaling/pipeline/$graph
        echo "    Running $graph"
        mpirun -n $num_gpus --map-by node:PE=4 --hostfile $hostfile ./build/applications/async_multi_gpus/$model --graph $dataset_path/$graph --layers $num_layers --hunits $hunits --epoch $epoch --lr $lr --decay $decay --part model --chunks $chunks --weight_file /tmp/saved_weights_pipe --dropout $dropout --seed $seed --eval_freq $eval_freq --exact_inference $exact_inference --num_dp_ways $num_dp_ways --enable_compression $enable_compression --multi_label $multi_label > ./nsdi2023/weak_scaling/pipeline/$graph/$num_gpus.txt 2>&1
    done
done
