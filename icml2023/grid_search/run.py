...
def train_gcn(lr, decay, hunit, dropout, full_graph_path, graph, weight_file):
    ...
    command = "mpirun --map-by node:PE=$SLURM_CPUS_PER_TASK ./build/applications/single_gpu/gcn --graph %s --layers %s --hunits %s --epoch 5000 --lr %s --decay %s --dropout %s --weight_file %s > %s 2>&1" % (
