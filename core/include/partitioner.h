...
//                        EdgeId num_edges = 0;
//                        for (VertexId v_i = curr_partition_begin; v_i < curr_partition_end; ++ v_i) {
...
//            EdgeId num_edges = graph_structure_->get_num_global_edges();
//            std::map<VertexId, VertexId> degree_dist;
//            std::vector<VertexId> possible_degree;
//            for (VertexId v_i = 0; v_i < num_vertices; ++ v_i) {
//                VertexId degree = graph_structure_->get_in_degree(v_i);
//                if (degree_dist.find(degree) == degree_dist.end()) {
//                    degree_dist[degree] = 0;
//                    possible_degree.push_back(degree);
//                }
//                degree_dist[degree] ++;
//            }
...
//            }
//
//            // find out the connection between high- and low-degree vertices
//            EdgeId num_high_high_edges = 0;
//            EdgeId num_high_low_edges = 0;
//            EdgeId num_low_low_edges = 0;
//            std::vector<VertexId> high_degree_vertices;
//            std::vector<VertexId> low_degree_vertices;
//            for (VertexId dst = 0; dst < num_vertices; ++ dst) {
//                VertexId dst_degree = graph_structure_->get_in_degree(dst);
//                if (dst_degree >= degree_threshold) {
//                    high_degree_vertices.push_back(dst);
//                } else {
//                    low_degree_vertices.push_back(dst);
//                }
//                InEdgeList in_edges = graph_structure_->get_in_edges(dst);
//                for (VertexId e_i = 0; e_i < in_edges.num_in_edges; ++ e_i) {
//                    VertexId src = in_edges.ptx[e_i].src;
//                    VertexId src_degree = graph_structure_->get_in_degree(src);
//                    if (dst_degree >= degree_threshold && src_degree >= degree_threshold) {
//                        num_high_high_edges ++;
//                    } else if (dst_degree < degree_threshold && src_degree < degree_threshold) {
//                        num_low_low_edges ++;
//                    } else {
//                        num_high_low_edges ++;
//                    }
...
//            printf("Number of high-high edges: %lu (%.3f)\n", num_high_high_edges, 1. * num_high_high_edges / num_edges);
//            printf("Number of low-low edges: %lu (%.3f)\n", num_low_low_edges, 1. * num_low_low_edges / num_edges);
//            printf("Number of high-low edges: %lu (%.3f)\n", num_high_low_edges, 1. * num_high_low_edges / num_edges);
//
//            // partition the high-low-vertices, independnetly
//            printf("With DP:\n");
//            VertexId num_low_degree_vertices = low_degree_vertices.size();
//            VertexId num_high_degree_vertices = high_degree_vertices.size();
//            graph_comm = 0;
//            for (int gpu = 0; gpu < num_gpus; ++ gpu) {
//                VertexId low_begin_idx = num_low_degree_vertices / num_gpus * gpu;
//                VertexId low_end_idx = num_low_degree_vertices / num_gpus * (gpu + 1);
//                if (gpu == num_gpus - 1) {
//                    low_end_idx = num_low_degree_vertices;
//                }
//                assert(num_gpus % 2 == 0);
//                VertexId high_begin_idx = num_high_degree_vertices / (num_gpus / 2) * (gpu / 2);
//                VertexId high_end_idx = num_high_degree_vertices / (num_gpus / 2) * (gpu / 2 + 1);
//                if (gpu / 2 == num_gpus / 2 - 1) {
//                    high_end_idx = num_high_degree_vertices;
//                }
...
//                    for (VertexId i = low_begin_idx; i < low_end_idx; ++ i) {
//                        local_vertices.insert(low_degree_vertices[i]);
//                    }
//                    for (VertexId i = high_begin_idx; i < high_end_idx; ++ i) {
//                        local_vertices.insert(high_degree_vertices[i]);
//                    }
//                    VertexId mirrors = num_mirror_vertices_calculator_->get_num_incoming_mirrors(local_vertices)
//                        + num_mirror_vertices_calculator_->get_num_outgoing_mirrors(local_vertices);
//                    if (gpu % 2 == 0) {
//                        graph_comm += sizeof(DataType) * mirrors * aggr_op_lengths[0];
//                    } else {
//                        graph_comm += sizeof(DataType) * mirrors * aggr_op_lengths[1];
//                    }
//                }
//                {
//                    std::set<VertexId> local_vertices;
...
//                }
//            }
//            inter_layer_comm = sizeof(DataType) * num_high_degree_vertices * inter_layer_embedding_size * 2;
//            printf("Inter-layer communication: %.3f MB\n", inter_layer_comm / 1024. / 1024.);
//            printf("Intra-layer communication: %.3f MB\n", graph_comm / 1024. / 1024.);
//            printf("Total: %.3f MB\n", (inter_layer_comm + graph_comm) / 1024. / 1024.);
//
//            return hybrid_comm;
//        }
//};
