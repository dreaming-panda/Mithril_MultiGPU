...
class CUDAPIPBackwardTaskDispatcher: public CUDAAbstractTaskDispatcher<CUDAPIPBackwardTask> {
    ...
    public:
        ...
        void set_cuda(cudnnHandle_t cudnn){
            cudnn_ = cudnn;
        }
};
...
{   
    ...
    public:
        ...
        LocalGraphBasic get_rowoffsets_in_(VertexId left, VertexId right){
            ...
            for(VertexId i = left + 1; i < right + 1; ++i){
                    row_offsets_in[i] = host_csrColIn_In_[i] - host_csrColIn_In_[left];
            }
            for(VertexId i = right + 1; i < num_master_vertices_ + 1; ++i){
                    row_offsets_in[i] = row_offsets_in[right];
            }
            int * cuda_rows = nullptr;
            int local_nnz = host_csrColIn_In_[right] - host_csrColIn_In_[left];
            ...
            int skip = host_csrColIn_In_[left];
            DataType * global_values = get_cuda_csrValue_In();
            DataType * local_values = global_values + skip;
            int  * global_cols = get_cuda_csrColIn_In();
        ...
        LocalGraphBasic get_rowoffsets_out_(VertexId left, VertexId right){
            printf("here !!!! \n");
            int * row_offsets_out = new int[num_master_vertices_ + 1];
            memset(row_offsets_out, 0, sizeof(int) * (num_master_vertices_ + 1));
            for(VertexId i = left + 1; i < right + 1; ++i){
                    row_offsets_out[i] = host_csrColIn_Out_[i] - host_csrColIn_Out_[left];
            }
            for(VertexId i = right + 1; i < num_master_vertices_ + 1; ++i){
                    row_offsets_out[i] = row_offsets_out[right];
            }
            int * cuda_rows = nullptr;
            int local_nnz = host_csrColIn_Out_[right] - host_csrColIn_Out_[left];
            ...
            int skip = host_csrColIn_Out_[left];
            DataType * global_values = get_cuda_csrValue_Out();
            DataType * local_values = global_values + skip;
            int  * global_cols = get_cuda_csrColIn_Out();
            int * local_cols = global_cols + skip;
            LocalGraphBasic lg;
            lg.cuda_local_rowoffsets = cuda_rows;
            lg.cuda_local_values = local_values;
