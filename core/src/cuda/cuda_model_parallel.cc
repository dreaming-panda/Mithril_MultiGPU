...
void DistributedModelParallelExecutionEngineGPU::partition_operators(
...
void DistributedModelParallelExecutionEngineGPU::execute_computation_graph_forward(
        const std::vector<Operator*> &operators,
        int * partition_assignments,
    ...
    for (std::pair<int, Tensor*> prev_tensor_pair: prev_tensors) {
        ...
        TensorResourceGPU * resource = (TensorResourceGPU*) remote_tensor->resource; 
    ...
    for (std::pair<int, Tensor*> suff_tensor_pair: suff_tensors) {
        ...
        TensorResourceGPU * resource = (TensorResourceGPU*) local_tensor->resource;
        assert(resource != NULL);
        DataType * data = resource->get_gpu_data();
        assert(data != NULL);
        size_t num_elements = resource->get_num_elements();
    ...
    for (Operator * op: operators) {
        ...
        for (int i = 0; i < num_output_tensors; ++ i) {
            ...
            if (tensor != output_tensor && // the tensor is not the output tensor
              ...
              //  DataType * cpugrad = resource->get_cpu_grad();
                size_t num_elements = resource->get_num_elements();
                assert(grad != NULL);
                assert(num_elements > 0);
              //  memset(cpugrad, 0, sizeof(DataType) * num_elements);
...
#pragma omp parallel for  
            for (size_t i = 0; i < num_elements_this_batch; ++ i) {
                cpugrad[num_received_elements + i] += comm_buff[i];
            }*/
   ...
   cudaStreamSynchronize(0);
    ...
    for (std::pair<int, Tensor*> prev_tensor_pair: prev_tensors) {
        ...
        assert(grad != NULL);
        assert(num_elements > 0);
        //CopyFromCUDADeviceToHost<DataType>(cpugrad, grad, num_elements, __FILE__, __LINE__);
        ...
        while (num_sent_elements < num_elements) {
            ...
            //printf("Node %d, Number of elements this batch: %llu\n", node_id, num_elements_this_batch);
          //  MPI_Send(
            //       cpugrad + num_sent_elements, num_elements_this_batch, 
              //      DistributedSys::get_mpi_data_type<DataType>(),
...
void DistributedModelParallelExecutionEngineGPU::get_boundary_operators(
                const std::vector<Operator*> &operators,
                const std::map<Operator*, int> &op_to_idx,
                int * partition_assignments,
...
double DistributedModelParallelExecutionEngineGPU::execute_application(
