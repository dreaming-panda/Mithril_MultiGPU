...
// DistributedPipelinedModelParallelExecutionEngineCPU

void DistributedPipelinedLinearModelParallelExecutionEngineCPU::forwarding_tasks_generator_thread_main(
        int num_epoch,
    ...
    if (node_id == 0) {
        ...
        for (int i = 0; i < num_epoch; ++ i) {
            ...
            for (std::pair<int, Tensor*> prev_tensor_pair: prev_tensors) {
                ...
                MultiVersionedTensorResourceCPU * resource = 
                    (MultiVersionedTensorResourceCPU*) remote_tensor->resource;
                assert(resource != NULL);
                DataType * data = resource->get_data(task.tensor_version);
                assert(data != NULL);
...
void DistributedPipelinedLinearModelParallelExecutionEngineCPU::forwarding_task_finalizer_thread_main(
        int num_epoch,
    ...
    if (node_id < num_nodes - 1) {
        ...
        for (int i = 0; i < num_epoch; ++ i) {
            ...
            for (std::pair<int, Tensor*> suff_tensor_pair: suff_tensors) {
                ...
                assert(tensor != NULL);
                MultiVersionedTensorResourceCPU * resource = 
                    (MultiVersionedTensorResourceCPU*) tensor->resource;
                assert(resource != NULL);
                DataType * data = resource->get_data(task.tensor_version);
                assert(data != NULL);
                size_t num_elements = resource->get_num_elements(task.tensor_version);
...
void DistributedPipelinedLinearModelParallelExecutionEngineCPU::backwarding_task_generator_thread_main(
        int num_epoch, 
        const std::vector<std::pair<int, Tensor*>> &suff_tensors
        ) {
        ...
        for (int i = 0; i < num_epoch; ++ i) {
            ...
            for (std::pair<int, Tensor*> suff_tensor_pair: suff_tensors) {
                ...
                MultiVersionedTensorResourceCPU * resource = 
                    (MultiVersionedTensorResourceCPU*) tensor->resource;
                assert(resource != NULL);
                DataType * grad = resource->get_grad(back_task.tensor_version);
                assert(grad != NULL);
...
void DistributedPipelinedLinearModelParallelExecutionEngineCPU::backwarding_task_finalizer_thread_main(
        int num_epoch, 
        const std::vector<std::pair<int, Tensor*>> &prev_tensors
        ) {
...
double DistributedPipelinedLinearModelParallelExecutionEngineCPU::start_training(
    ...
    while (task_id < 2 * num_epoch) {
        ...
        if (success) {
            ...
            for (Operator * op: weight_ops) {
                assert(op != NULL);
                TensorResourceCPU * resource = latest_weight_data.at(op);
                assert(resource != NULL);
                MultiVersionedTensorResourceCPU * multi_versioned_resource
                    = (MultiVersionedTensorResourceCPU*) op->get_output_tensor(0)->resource;
            ...
            for (int op_idx = 0; op_idx < num_operators; ++ op_idx) {
                ...
                for (int i = 0; i < num_input_tensors; ++ i) {
                    ...
                    MultiVersionedTensorResourceCPU * resource = 
                        (MultiVersionedTensorResourceCPU*) input_tensor->resource;
                    TensorResourceCPU * shadow_resource = 
                        (TensorResourceCPU*) shadow_input_tensor->resource;
                    assert(resource != NULL);
                    assert(shadow_resource != NULL);
                    assert(shadow_resource->get_data() == NULL);
                    shadow_resource->set_data(
                            resource->get_data(forwarding_task.tensor_version)
                            );
                }
                ...
                for (int i = 0; i < num_output_tensors; ++ i) {
                    ...
                    MultiVersionedTensorResourceCPU * resource = 
                        (MultiVersionedTensorResourceCPU*) output_tensor->resource;
                    TensorResourceCPU * shadow_resource = 
                        (TensorResourceCPU*) shadow_output_tensor->resource;
                    assert(resource != NULL);
                    assert(shadow_resource != NULL);
                    assert(shadow_resource->get_data() == NULL);
                    shadow_resource->set_data(
            ...
            if (node_id == num_nodes - 1) {
                ...
                assert(shadow_resource->get_data() == NULL);
                assert(shadow_resource->get_grad() == NULL);
                shadow_resource->set_data(
                        resource->get_data(forwarding_task.tensor_version)
                        );
                shadow_resource->set_grad(
                        resource->get_grad(forwarding_task.tensor_version)
                        );
        ...
        if (success) {
            ...
            for (Tensor * t: tensors_need_init) {
                MultiVersionedTensorResourceCPU * resource = 
                    (MultiVersionedTensorResourceCPU*) t->resource;
                assert(resource != NULL);
                memset(
            ...
            for (Operator * op: weight_ops) {
                ...
                MultiVersionedTensorResourceCPU * multi_versioned_resource =
                    (MultiVersionedTensorResourceCPU*) op->get_output_tensor(0)->resource;
                assert(multi_versioned_resource != NULL);
                DataType * grad = multi_versioned_resource->get_grad(
                        backwarding_task.tensor_version
                        );
                TensorResourceCPU * resource = latest_weight_data.at(op);
                assert(resource != NULL);
...
void DistributedPipelinedLinearModelParallelExecutionEngineCPU::prepare_input_tensor(Tensor * input_tensor) {
    // fake a single-version tensor resource first
    MultiVersionedTensorResourceCPU * multi_versioned_resource = 
        (MultiVersionedTensorResourceCPU*) input_tensor->resource;
    VertexId num_vertices = multi_versioned_resource->get_num_vertices();
    TensorResourceCPU * single_versioned_resource = new TensorResourceCPU(input_tensor, num_vertices);
    single_versioned_resource->set_data(multi_versioned_resource->get_data(0));
    input_tensor->resource = single_versioned_resource;

    // invoke the prepare_input_tensor function of the super class 
    SingleNodeExecutionEngineCPU::prepare_input_tensor(input_tensor);
...
double DistributedPipelinedLinearModelParallelExecutionEngineCPU::execute_application(
    ...
    for (int op_idx = 0; op_idx < num_operators; ++ op_idx) {
        if (partition_assignments[op_idx] == node_id) {
            ...
            for (int i = 0; i < num_output_tensors; ++ i) {
                ...
                assert(tensor != NULL);
                assert(tensor->resource == NULL);
                tensor->resource = new MultiVersionedTensorResourceCPU(
                        tensor, 
    ...
    std::map<Operator*, TensorResourceCPU*> latest_weight_data;
    weight_ops.clear();
    for (Operator * op: operators) {
        if (op->get_type() == OPERATOR_WEIGHT && op_to_partition[op] == node_id) {
            ...
            TensorResourceCPU * resource = new TensorResourceCPU(
    ...
    for (Operator * op: operators) {
        ...
        for (int i = 0; i < num_output_tensors; ++ i) {
            ...
            shadow_tensor->resource = new TensorResourceCPU(shadow_tensor,
                    graph_structure_->get_num_global_vertices());
            assert(shadow_tensor->resource != NULL);
    ...
    for (Operator * op: weight_ops) {
        TensorResourceCPU * resource = latest_weight_data[op];
        resource->unmap();
        delete resource;
    }
...
void DistributedPipelinedLinearModelParallelWithGraphChunkingExecutionEngineCPU::forwarding_tasks_generator_thread_main(
        int num_epoch,
        const std::vector<std::pair<int, Tensor*>> &prev_tensors
        ) {
    // this thread is responsible for generating pending forwarding tasks
...
void DistributedPipelinedLinearModelParallelWithGraphChunkingExecutionEngineCPU::forwarding_task_finalizer_thread_main(
        int num_epoch,
        const std::vector<std::pair<int, Tensor*>> &suff_tensors
        ) {
    // this thread is responsible for passing the forwarding task to the next node
...
void DistributedPipelinedLinearModelParallelWithGraphChunkingExecutionEngineCPU::backwarding_task_generator_thread_main(
        int num_epoch, 
        const std::vector<std::pair<int, Tensor*>> &suff_tensors
        ) {
    // this thread is responsible for generating backwarding tasks
...
void DistributedPipelinedLinearModelParallelWithGraphChunkingExecutionEngineCPU::backwarding_task_finalizer_thread_main(
        int num_epoch, 
        const std::vector<std::pair<int, Tensor*>> &prev_tensors
        ) {
    // this thread is responsible for passing the finished backwarding task to the previous node
...
// returned: accuracy
double DistributedPipelinedLinearModelParallelWithGraphChunkingExecutionEngineCPU::start_training(
        const std::vector<Operator*>& operators,
        const std::map<Operator*, int>& op_to_idx,
        const std::map<Operator*, int>& op_to_partition,
        Tensor * input_tensor,
...
double DistributedPipelinedLinearModelParallelWithGraphChunkingExecutionEngineCPU::execute_application(AbstractApplication * application, int num_epoch) {
    assert(application != NULL);
    const std::vector<Operator*>& operators = application->get_operators();
    int num_operators = operators.size();
