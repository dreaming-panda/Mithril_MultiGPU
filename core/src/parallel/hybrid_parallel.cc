...
PIPForwardTaskDispatcher::PIPForwardTaskDispatcher(
    ...
    AbstractTaskDispatcher<PIPForwardTask>(max_num_tasks, barrier) {
...
void PIPForwardTaskDispatcher::thread_main() {
    ...
    PIPForwardTask task;
    DataDependenciesTracker * data_dependencies_tracker = engine_->get_data_dependencies_tracker();
    ...
    if (engine_->is_topmost_node()) {
        ...
        for (int epoch_id = 0; epoch_id < num_epoch; ++ epoch_id) {
            ...
            while (num_dispatched_chunks < num_local_chunks) {
                MPI_Status status;
                MPI_Recv(
                        &task, sizeof(PIPForwardTask), MPI_CHAR, 
...
PIPBackwardTaskDispatcher::PIPBackwardTaskDispatcher(
        int max_num_tasks,
        pthread_barrier_t * barrier): 
    AbstractTaskDispatcher<PIPBackwardTask>(max_num_tasks, barrier) {
        num_ready_remote_nodes_ = new std::map<int, int>();
        assert(num_ready_remote_nodes_ != NULL);
        input_task_queue_ = new LockFreeQueue<PIPBackwardTask>(max_num_tasks);
...
LockFreeQueue<PIPBackwardTask> * PIPBackwardTaskDispatcher::get_input_task_queue() {
    return input_task_queue_;
}

void PIPBackwardTaskDispatcher::insert_new_task(PIPBackwardTask task) { // only works for bottommost nodes
    assert(engine_->is_bottommost_node());
    input_task_queue_->push(task);
}

void PIPBackwardTaskDispatcher::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
    ...
    if (engine_->is_bottommost_node()) {
        ...
        for (int epoch_id = 0; epoch_id < num_epoch; ++ epoch_id) {
            ...
            ShadowGradientsMasterVertices * shadow_gradients = 
            ...
            while (num_dispatched_chunks < num_local_chunks) {
                ...
                MPI_Status status;
                MPI_Recv(
                        &task, sizeof(PIPBackwardTask), MPI_CHAR,
...
PIPForwardTaskCommitter::PIPForwardTaskCommitter(
        int max_num_tasks,
        pthread_barrier_t * barrier
        ): 
    AbstractTaskCommitter<PIPForwardTask>(max_num_tasks, barrier) {
}

PIPForwardTaskCommitter::~PIPForwardTaskCommitter() {
}

void PIPForwardTaskCommitter::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
...
PIPBackwardTaskCommitter::PIPBackwardTaskCommitter(
        int max_num_tasks,
        pthread_barrier_t * barrier
        ): 
    AbstractTaskCommitter<PIPBackwardTask>(max_num_tasks, barrier) {
}

PIPBackwardTaskCommitter::~PIPBackwardTaskCommitter() {
}

void PIPBackwardTaskCommitter::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
                ...
                for (int remote_node: *remote_nodes) {
                    ...
                    for (Tensor * tensor: *dependencies) {
                        ...
                        while (num_sent_elements < num_elements_this_chunk) {
                            ...
                            MPI_Send(
                                    grad + num_sent_elements, num_elements_to_send,
                                    DistributedSys::get_mpi_data_type<DataType>(),
                                    remote_node, BackwardGradientPassing,
                                    MPI_COMM_WORLD
                                    );
...
AbstractPIPScheduler::AbstractPIPScheduler(
...
PIP1Forward1BackwardPrioritizedUpdateScheduler::PIP1Forward1BackwardPrioritizedUpdateScheduler(
        DistributedPIPHybridParallelExecutionEngineCPU * engine,
        PIPForwardTaskDispatcher * forward_task_dispatcher,
        PIPForwardTaskCommitter * forward_task_committer,
        PIPBackwardTaskDispatcher * backward_task_dispatcher,
...
void PIP1Forward1BackwardPrioritizedUpdateScheduler::schedule_task() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();

    LockFreeQueue<PIPForwardTask> * forward_task_dispatcher_queue_ = forward_task_dispatcher_->get_task_queue();
    LockFreeQueue<PIPForwardTask> * forward_task_committer_queue_ = forward_task_committer_->get_task_queue();
    LockFreeQueue<PIPBackwardTask> * backward_task_dispatcher_queue_ = backward_task_dispatcher_->get_task_queue();
    LockFreeQueue<PIPBackwardTask> * backward_task_committer_queue_ = backward_task_committer_->get_task_queue();
...
bool OperatorsAndTensorsManager::is_operator_list_ordered() {
...
void OperatorsAndTensorsManager::build_ordered_tensor_list() {
...
OperatorsAndTensorsManager::OperatorsAndTensorsManager(const std::vector<Operator*>& operators):
...
OperatorsAndTensorsManager::~OperatorsAndTensorsManager() {
}

// VertexIdTranslationTable

VertexIdTranslationTable::VertexIdTranslationTable(
...
VertexTensorDataGradManager::VertexTensorDataGradManager(
...
VertexChunksManager::VertexChunksManager(
        AbstractGraphStructure * graph, 
...
bool PIPPartitioner::is_valid_partition(PIPPartitioning p, VertexId num_global_vertices, int num_operators) {
...
void DataDependenciesTracker::build_p_link_dependencies(int fragment_id) {
...
// how to determine whether a remote node depends on the graph data of the local node
// sufficient and necessary conditions:
// 1) the remote node has at least one mirror vertex that belongs to the local node
// 2) a) for activation updating, the remote node has at least one aggregation operator,
//       whose input tensor is an output tensor of a local operator (produced by the local node);
//    b) for gradient updating, the remote node has at least one aggregation operator,
//       whose output tensor is an input tensor of a local operator (its grad produced by the 
//       local node);
void DataDependenciesTracker::build_i_link_dependencies() {
...
void DataDependenciesTracker::build_i_link_activation_sender_dependencies() {
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int node_id = DistributedSys::get_instance()->get_node_id();
...
void DataDependenciesTracker::build_i_link_activation_receiver_dependencies() {
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int node_id = DistributedSys::get_instance()->get_node_id();
    VertexId local_vid_begin = partitioning_.partition_vid_begin[node_id];
    VertexId local_vid_end = partitioning_.partition_vid_end[node_id];
...
void DataDependenciesTracker::build_i_link_gradient_sender_dependencies() {
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int node_id = DistributedSys::get_instance()->get_node_id();
    VertexId local_vid_begin = partitioning_.partition_vid_begin[node_id];
    VertexId local_vid_end = partitioning_.partition_vid_end[node_id];
...
void DataDependenciesTracker::build_i_link_gradient_receiver_dependencies() {
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int node_id = DistributedSys::get_instance()->get_node_id();
    VertexId local_vid_begin = partitioning_.partition_vid_begin[node_id];
    VertexId local_vid_end = partitioning_.partition_vid_end[node_id];
...
DataDependenciesTracker::DataDependenciesTracker(
    ...
    assert(PIPPartitioner::is_valid_partition(partitioning, num_global_vertices, num_operators)); // make sure that the partitioning is valid
...
int DataDependenciesTracker::get_num_activation_updates_to_recv() {
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int node_id = DistributedSys::get_instance()->get_node_id();
...
int DataDependenciesTracker::get_num_gradient_updates_to_recv() {
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_chunks = chunk_manager_->get_num_global_chunks();
...
PIPLocalGraph::PIPLocalGraph(AbstractGraphStructure * global_graph, VertexIdTranslationTable * vid_translation) {
...
PIPLocalGraph::~PIPLocalGraph(){
    destroy();
}

// PIPGraphDataActivationUpdateSender

void PIPGraphDataActivationUpdateSender::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
                        ...
                        while (vid < chunk_end && 
                            ...
                            if (engine_->has_incoming_mirror(vid, remote_node)) {
                                memcpy(
                                        ...
                                        data + (vid - chunk_begin) * num_elements_per_vertex,
...
void PIPGraphDataActivationUpdateReceiver::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
...
void PIPGraphDataGradientUpdateSender::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
                    ...
                    while (num_sent_elements < num_elements_should_be_sent) {
                        ...
                        while (vid < chunk_end && 
                            ...
                            if (engine_->has_outgoing_mirror(vid, remote_node)) {
                                memcpy(
                                        comm_buff + num_elements_to_send,
                                        grad + (vid - chunk_begin) * num_elements_per_vertex,
                                        sizeof(DataType) * num_elements_per_vertex
                                      );
                                num_elements_to_send += num_elements_per_vertex;
                            }
...
void PIPGraphDataGradientUpdateReceiver::thread_main() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int num_nodes = DistributedSys::get_instance()->get_num_nodes();
    int num_epoch = engine_->get_num_epoch();
    const std::vector<int>& local_chunk_ids = engine_->get_local_chunk_ids();
...
void PIPParallelParameterServer::data_pulling_request_handling_thread_main() {
    PIPPSHeader header;
    ...
    while (true) {
        ...
        MPI_Irecv(
                &header, sizeof(PIPPSHeader), MPI_CHAR,
        ...
        MPI_Send(
                p.first, num_elements, DistributedSys::get_mpi_data_type<DataType>(),
...
void PIPParallelParameterServer::grad_pushing_handling_thread_main() {
    PIPPSHeader header;
    ...
    while (true) {
        ...
        std::pair<DataType*, DataType*> p = weight_data_grad_[weight_op];
        MPI_Recv(
                p.second, num_elements, DistributedSys::get_mpi_data_type<DataType>(),
...
PIPParallelParameterServer::PIPParallelParameterServer(
        OperatorsAndTensorsManager * op_ten_manager,
    ...
    for (int op_idx = 0; op_idx < num_operators; ++ op_idx) {
        ...
        if (master_node == node_id) {
            ...
            engine->init_weight_tensor_data(data, num_elements, tensor->dims[0]);
        }
        weight_op_idx ++;
    }
...
void PIPParallelParameterServer::pull_weight(WeightOperator * weight_op, DataType * data) {
...
void PIPParallelParameterServer::push_grad(WeightOperator * weight_op, DataType * grad) {
    int master_node = master_nodes_[weight_op];
    int node_id = DistributedSys::get_instance()->get_node_id();
    Tensor * tensor = weight_op->get_output_tensor(0);
    size_t num_elements = 1;
...
DistributedPIPHybridParallelExecutionEngineCPU::DistributedPIPHybridParallelExecutionEngineCPU() {
}

DistributedPIPHybridParallelExecutionEngineCPU::~DistributedPIPHybridParallelExecutionEngineCPU() {
}

void DistributedPIPHybridParallelExecutionEngineCPU::perform_forward_task(PIPForwardTask task) {
    ...
    for (WeightOperator * op: local_weight_ops_) {
        ...
        parameter_server_->pull_weight(op, resource->get_data()); 
        // stash the weight
...
void DistributedPIPHybridParallelExecutionEngineCPU::perform_backward_task(PIPBackwardTask task) {
    int chunk_id = task.chunk_id;
    int node_id = DistributedSys::get_instance()->get_node_id();
    VertexId global_vid_begin = chunk_manager_->get_chunk_begin(chunk_id);
    VertexId global_vid_end = chunk_manager_->get_chunk_end(chunk_id);
    ...
    for (WeightOperator * op: local_weight_ops_) { 
        ...
        assert(tensor != NULL);
        TensorResourceCPU * resource = (TensorResourceCPU*) tensor->resource;
        parameter_server_->push_grad(op, resource->get_grad());
    }
...
void DistributedPIPHybridParallelExecutionEngineCPU::generate_backward_operator_mask(
...
void DistributedPIPHybridParallelExecutionEngineCPU::init_weights() {
    int node_id = DistributedSys::get_instance()->get_node_id();
    int local_op_begin = partitioning_.partition_op_begin[node_id];
    int local_op_end = partitioning_.partition_op_end[node_id];
...
void DistributedPIPHybridParallelExecutionEngineCPU::prepare_input_tensor() {
...
void DistributedPIPHybridParallelExecutionEngineCPU::prepare_std_tensor() {
    ...
    std_tensor_->op = NULL;
    std_tensor_->idx = -1;
    std_tensor_->resource = new TensorResourceCPU(
            std_tensor_, vid_translation_->get_num_master_vertices()
            );
    std_tensor_->resource->map();

    TensorResourceCPU * resource = (TensorResourceCPU*) std_tensor_->resource;
...
void DistributedPIPHybridParallelExecutionEngineCPU::set_up_tensor_resourses() {
    ...
    for (int i = 0; i < num_tensors; ++ i) {
        ...
        assert(tensor != NULL);
        TensorResourceCPU * resource = new TensorResourceCPU(tensor, num_local_vertices); 
...
void DistributedPIPHybridParallelExecutionEngineCPU::release_resources() {
    for (WeightOperator * weight_op: local_weight_ops_) {
        assert(weight_op->get_num_output_tensors() == 1);
        Tensor * tensor = weight_op->get_output_tensor(0);
    ...
    for (int i = 0; i < num_tensors; ++ i) {
        Tensor * tensor = op_ten_manager_->get_tensor(i);
        TensorResourceCPU * resource = (TensorResourceCPU*) tensor->resource;
        resource->set_data(NULL);
        resource->set_grad(NULL);
        delete resource;
    }
}

void DistributedPIPHybridParallelExecutionEngineCPU::calculate_accuracy_and_loss() {
    // calculate the accuracy
    double accuracy = 0.;
    if (is_bottommost_node_) {
        accuracy = SingleNodeExecutionEngineCPU::calculate_accuracy(output_tensor_, std_tensor_);
    }
...
double DistributedPIPHybridParallelExecutionEngineCPU::execute_application(AbstractApplication * application, int num_epoch) {
    ...
    assert(num_nodes == 1);
    partitioning.partition_vid_begin[0] = 0;
    partitioning.partition_vid_end[0] = num_global_vertices;
    partitioning.partition_op_begin[0] = 0;
    partitioning.partition_op_end[0] = num_operators;
    */
    ...
    assert(num_nodes == 2);
    partitioning.partition_vid_begin[0] = 0;
    partitioning.partition_vid_end[0] = num_global_vertices / 2;
    partitioning.partition_vid_begin[1] = num_global_vertices / 2;
    partitioning.partition_vid_end[1] = num_global_vertices;
    partitioning.partition_op_begin[0] = 0;
    partitioning.partition_op_end[0] = num_operators;
    ...
    */
    ...
    partitioning.partition_vid_begin[0] = 0;
    partitioning.partition_vid_end[0] = num_global_vertices;
    partitioning.partition_vid_begin[1] = 0;
    partitioning.partition_vid_end[1] = num_global_vertices;
    partitioning.partition_op_begin[0] = 0;
    partitioning.partition_op_end[0] = num_operators / 2 + (num_operators % 2 == 1);
    partitioning.partition_op_begin[1] = num_operators / 2 + (num_operators % 2 == 1);
    partitioning.partition_op_end[1] = num_operators;
    */
    ...
    for (int i = 0; i < 4; ++ i) {
        ...
        partitioning.partition_vid_end[i] = num_global_vertices;
        partitioning.partition_op_begin[i] = (num_operators / 4) * i;
        partitioning.partition_op_end[i] = (num_operators / 4) * (i + 1);
    }
    partitioning.partition_op_end[3] = num_operators;
    */
    ...
    op_ten_manager_ = new OperatorsAndTensorsManager(operators);
    vid_translation_ = new VertexIdTranslationTable(
            graph_structure_, 
            partitioning.partition_vid_begin[node_id], partitioning.partition_vid_end[node_id]
            );
    vtensor_manager_ = new VertexTensorDataGradManager(
    ...
    shadow_gradients_ = new ShadowGradientsMasterVertices(vid_translation_, chunk_manager_);
    //local_graph_ = new PIPLocalGraph(graph_structure_, vid_translation_);
    CUDAPIPLocalGraph * lgraph = new CUDAPIPLocalGraph(graph_structure_, vid_translation_);
    lgraph->InitMemory();
    lgraph->InitCsr();
    local_graph_ = lgraph;
    parameter_server_ = new PIPParallelParameterServer(op_ten_manager_, optimizer_->get_lower_level_optimizer(), this);
    ...
    forward_task_dispatcher_ = new PIPForwardTaskDispatcher(total_num_forwarding_tasks, &barrier_);
    forward_task_committer_ = new PIPForwardTaskCommitter(total_num_forwarding_tasks, &barrier_);
    backward_task_dispatcher_ = new PIPBackwardTaskDispatcher(total_num_backwarding_tasks, &barrier_);
    backward_task_committer_ = new PIPBackwardTaskCommitter(total_num_backwarding_tasks, &barrier_);
    act_update_sender_ = new PIPGraphDataActivationUpdateSender(this, total_num_forwarding_tasks, &barrier_);
    act_update_receiver_ = new PIPGraphDataActivationUpdateReceiver(this, &barrier_);
    grad_update_sender_ = new PIPGraphDataGradientUpdateSender(this, total_num_backwarding_tasks, &barrier_); 
    grad_update_receiver_ = new PIPGraphDataGradientUpdateReceiver(this, &barrier_);
    ...
    weight_stashing_manager_ = new WeightStashingManager(local_weight_ops_);
    assert(weight_stashing_manager_ != NULL);

    // start task scheduling
    scheduler_ = new PIP1Forward1BackwardPrioritizedUpdateScheduler(
...
void CUDAPIPLocalGraph::InitCsr()
{
...
void CUDAPIPLocalGraph::TestCsr()
{   
    int node_id = DistributedSys::get_instance()->get_node_id();
